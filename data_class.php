...
class data extends db {
    ...
    function addnewuser($name,$pasword,$email,$type){
        ...
        if($this->connection->exec($q)) {
            header("Location:librarian_service_dashboard.php?msg=New Add done");
        }
        else {
            header("Location:librarian_service_dashboard.php?msg=Register Fail");
        }
    }
    ...
    function librarianLogin($t1, $t2) {
        $q="SELECT * FROM librarian where email='$t1' and pass='$t2'";
        $recordSet=$this->connection->query($q);
        $result=$recordSet->rowCount();
        ...
        if ($result > 0) {
            foreach($recordSet->fetchAll() as $row) {
                $logid=$row['id'];
                $_SESSION["librarianid"] = $logid;
                header("location: librarian_service_dashboard.php");
            }
        }
        else {
            header("location: index.php?msg=Invalid Credentials");
    ...
    function addbook($bookpic, $bookname, $bookdetail, $bookauthor, $bookpub, $branch, $bookprice, $bookquantity) {
       ...
       $q="INSERT INTO book (id,bookpic,bookname,bookdetail,bookauthor,bookpub,branch,bookprice,bookquantity,bookava,bookrent)VALUES('','$bookpic', '$bookname', '$bookdetail', '$bookauthor', '$bookpub', '$branch', '$bookprice', '$bookquantity','$bookquantity','0')";
        if($this->connection->exec($q)) {
            header("Location:librarian_service_dashboard.php?msg=done");
        }
        else {
            header("Location:librarian_service_dashboard.php?msg=fail");
        }
    }
      ...
      function issuebookapprove($book,$userselect,$days,$getdate,$returnDate,$redid){
        ...
        if ($result > 0) {
            foreach($recordSet->fetchAll() as $row) {
                $issueid=$row['id'];
                $issuetype=$row['type'];
                // header("location: librarian_service_dashboard.php?logid=$logid");
            }
            foreach($recordSetss->fetchAll() as $row) {
                $bookid=$row['id'];
                $bookname=$row['bookname'];
